...
//
//
#ifndef __QTFidITERATOR_H__
#define __QTFidITERATOR_H__
...
class CQTFidIterator : public ParsedFidIterator
{
	enum {
		ULong = 1,
		UFloat, 
...
public :
	// -------------------------------------------------------------------------
	// Default Constructor
	CQTFidIterator(const byte* pbyMsg):ParsedFidIterator(pbyMsg){}
...
public :
	 ...
	 void  SetQuoteListPageFid(long lPage, short sFid);
public:
	bool ProcessFid();
	...
	void PrintBDateYear(BDateYear& dy, CString& strOutput);
	void PrintULong(unsigned long& ul, CString& strOutput);
	void PrintFloat(float& f, CString& strOutput);
	void PrintCharArray(CharArray& caOld, CString& strOutput);
	void PrintGBCharArray(CharArray& caOld, CString& strOutput);
	void PrintChar(char& c, CString& strOutput);
	void PrintUDouble(double& d, CString& strOutput);
	void PrintBTimeSec(BTimeSec& ts, CString& strOutput);

	void PrintQuote(int iFidType, unsigned char& yLen, void*& pFV);
	void PrintQuote(int iFidType, unsigned char& yLen, void*& pFV, int iRow, int iCol);
